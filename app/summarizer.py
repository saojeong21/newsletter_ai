...
async def _call_openrouter(
    ...
    prompt_template: str = SUMMARY_PROMPT_KO,
) -> Optional[str]:
    ...
    Args:
        ...
        prompt_template: 사용할 프롬프트 템플릿 (한국어/영어)
...
async def summarize_article(
    ...
    """무료 모델 순차 폴백으로 기사를 요약한다.

    국내 소스(AI타임스, 한국경제 IT 등)는 한국어로, 해외 소스는 영어로 요약한다.
    ...
    lang = "한국어" if _is_korean_source(source_name) else "영어"
    log_prefix = f"[Article #{article_id}]" if article_id else "[Article]"
    logger.info(f"{log_prefix} 요약 언어: {lang} (소스: {source_name})")

    async with httpx.AsyncClient() as client:
        for i, model in enumerate(FREE_MODELS):
            try:
                logger.info(f"{log_prefix} 요약 시도: {model} ({i+1}/{len(FREE_MODELS)})")
                summary = await _call_openrouter(title, content, model, client, prompt_template)
